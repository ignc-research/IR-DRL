...
class listener_node_one:
    ...
    def PointcloudToVoxel(self):
        ...
        # get voxel_clusters
        voxel_clusters = self.get_voxel_cluster(voxel_centers,self.neighbourhood_threshold)
    ...
    def get_voxel_cluster(self, voxel_centers, neighbourhood_threshold):
        # calculate for every voxel which cluster they belong to
        # returns array which assigns each voxel a cluster number via idx
        # TODO use set
        ...
        for i in range(len(voxel_centers)):
            if voxel_cluster[i] < 0:
                # no cluster assigned
                self.set_clusters(i, voxel_centers, voxel_cluster, max_cluster_num, neighbourhood_threshold)
                # TODO check for assign by reference or value error
                max_cluster_num += 1
        return voxel_cluster

    def set_clusters(self, initial_voxel_idx, voxel_centers, voxel_cluster, cluster_num, neighbourhood_threshold):
        # breadth first search
        # assigns cluster_num, to voxels belonging to same cluster as initial_voxel
        ...
        while queue:
            ...
            if voxel_cluster[idx] < 0: 
                ...
                neighbors = self.get_neighbouring_voxels_idx(voxel_centers, current_voxel, neighbourhood_threshold)
                queue.extend(neighbors)

    def get_neighbouring_voxels_idx(self, voxel_centers, voxel, neighbour_threshold):
        # returns indices of voxels that are within neighbouring threshold
        voxels_in_cluster_bool = np.linalg.norm((voxel_centers - voxel), axis=1) <= neighbour_threshold
        voxels_in_cluster_idx = np.array([i for i in range(voxels_in_cluster_bool.shape[0])])[voxels_in_cluster_bool]
