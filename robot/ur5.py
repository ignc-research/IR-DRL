...
class UR5(Robot):
    ...
    def __init__(self, name: str, world, use_physics_sim: bool, base_position: Union[list, np.ndarray], base_orientation: Union[list, np.ndarray], resting_angles: Union[list, np.ndarray], end_effector_link_id: int, base_link_id: int, control_mode: int, xyz_delta:float, rpy_delta:float, joint_delta:float):
        super().__init__(name, world, use_physics_sim, base_position, base_orientation, resting_angles, end_effector_link_id, base_link_id, control_mode, xyz_delta, rpy_delta, joint_delta)
